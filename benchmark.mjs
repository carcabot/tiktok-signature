    ...
    if (data.status === "ok") {
      ...
      if (endpoint === "fetch") {
        ...
        return {
          success: hasData,
          duration,
          items: data.data?.itemList?.length || 0,
        };
      }
    }
...
async function runBenchmark(args) {
  ...
  try {
    ...
    if (!health.ready) {
      console.log(
        "ERROR: Server is not ready. Please wait for initialization.",
      );
      process.exit(1);
    }
  ...
  const showProgress = () => {
    ...
    process.stdout.write(
      `\r[${bar}] ${pct}% (${completed}/${args.requests}) - ${successful} ok, ${failed} failed`,
    );
  };
