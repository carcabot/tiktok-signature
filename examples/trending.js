...
function displayResults(data) {
  ...
  data.itemList.forEach((video, index) => {
    ...
    console.log(
      `   Author: @${video.author?.uniqueId || "Unknown"} ${video.author?.verified ? "(Verified)" : ""}`,
    );
    console.log(
      `   Description: ${(video.desc || "No description").substring(0, 55)}${video.desc?.length > 55 ? "..." : ""}`,
    );
    ...
    if (video.stats) {
      ...
      console.log(
        `   Views: ${viewsFormatted} | Likes: ${likesFormatted} | Comments: ${video.stats.commentCount?.toLocaleString() || "N/A"}`,
      );
    }

    if (video.music?.title) {
      console.log(
        `   Music: ${video.music.title.substring(0, 40)}${video.music.title.length > 40 ? "..." : ""}`,
      );
    }
