...
function displayUserInfo(data, targetUsername) {
  ...
  const exactMatch = users.find(item => {
    ...
    return (user.uniqueId || user.unique_id || '').toLowerCase() === targetUsername.toLowerCase();
  });
  ...
  console.log(`SecUid:       ${(user.secUid || user.sec_uid || '').substring(0, 40)}...`);
  console.log(`Verified:     ${user.verified || user.custom_verify ? 'Yes' : 'No'}`);
  console.log('');
  ...
  console.log(`  Followers:    ${(user.followerCount || user.follower_count)?.toLocaleString() || 'N/A'}`);
  console.log(`  Following:    ${(user.followingCount || user.following_count)?.toLocaleString() || 'N/A'}`);
  console.log(`  Total Likes:  ${(user.heartCount || user.total_favorited)?.toLocaleString() || 'N/A'}`);
  console.log('');
  ...
  if (!exactMatch && users.length > 1) {
    ...
    users.slice(1, 5).forEach((item, index) => {
      ...
      console.log(`${index + 2}. @${u.uniqueId || u.unique_id} - ${u.nickname}`);
    });
  }
